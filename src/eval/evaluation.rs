...
pub fn score_position(position: &Position) -> isize {
    ...
    let mut score = blended_score + material_score + score_bishops(position) + score_rooks(position);
...
pub fn is_drawn_by_agreement(position: &Position, repetition_key_stack: &[RepetitionKey]) -> bool {
    position.is_drawn_by_fifty_moves_rule()
        || position_occurrence_count(repetition_key_stack) >= 3
        || is_drawn_by_insufficient_material(position)
}
...
pub fn is_drawn_by_insufficient_material(position: &Position) -> bool {
    let board = position.board();
    ...
    let has_insufficient_minor_pieces = |piece_color: PieceColor, knight_count: usize, bishop_count: usize| -> bool {
        knight_count == 2 || (bishop_count == 2 && board.has_bishops_on_same_color_squares(piece_color))
    };

    if blacks_minor_piece_count == 0
        && whites_minor_piece_count == 2
        && has_insufficient_minor_pieces(PieceColor::White, whites_knight_count, whites_bishop_count)
    {
        return true;
    }
    if whites_minor_piece_count == 0
        && blacks_minor_piece_count == 2
        && has_insufficient_minor_pieces(PieceColor::Black, blacks_knight_count, blacks_bishop_count)
    {
        return true;
    }
    false
...
pub fn get_game_status(position: &Position, repetition_key_stack: &[RepetitionKey]) -> GameStatus {
    ...
    match (!has_legal_move, check_count > 0) {
        ...
        _ => {
            ...
            } else if is_drawn_by_insufficient_material(position) {
...
fn calculate_material_balance(piece_counts: [[usize; 6]; 2]) -> [isize; 6] {
    ...
    for piece_type in PieceType::iter() {
        result[piece_type as usize] = piece_counts[PieceColor::White as usize][piece_type as usize] as isize
...
mod tests {
    ...
    mod insufficient_material {
        ...
        fn test_new_game() {
            ...
            assert_eq!(is_drawn_by_insufficient_material(&position), false);
        }
        ...
        fn test_only_kings() {
            ...
            assert_eq!(is_drawn_by_insufficient_material(&position), true);
        }
