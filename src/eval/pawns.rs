...
pub fn score_pawns(position: &Position) -> (isize, isize) {
    let score_mg = score_pawn_structure_mg(position, PieceColor::White) - score_pawn_structure_mg(position, PieceColor::Black);
    let score_eg = score_pawn_structure_eg(position, PieceColor::White) - score_pawn_structure_eg(position, PieceColor::Black);
    (score_mg, score_eg)
}
...
fn has_pawn_majority(board: &Board, piece_color: PieceColor, board_side: BoardSide) -> bool {
    let pawns = [board.bitboard_by_color_and_piece_type(PieceColor::White, PieceType::Pawn), board.bitboard_by_color_and_piece_type(PieceColor::Black, PieceType::Pawn)];
