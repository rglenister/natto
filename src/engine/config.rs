...
fn load_config() -> Config {
    dotenv().ok();
    CONFIG_OVERRIDE.lock().unwrap().clone().unwrap_or_else(|| {
        let matches = Command::new("Chess Engine")
            ...
            .group(ArgGroup::new("flags").args(["perft", "uci"]).required(false).multiple(false))
            .get_matches();
