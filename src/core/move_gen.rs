...
pub fn king_attacks_finder_empty_board(position: &Position, king_color: PieceColor) -> u64 {
    square_attacks_finder_empty_board(position, king_color.opposite(), position.board().king_square(king_color))
}
...
static PAWN_ATTACKS_TABLE: Lazy<[[u64; 64]; 2]> = Lazy::new(|| {
    ...
    fn generate_move_table(increments: [isize; 2]) -> [u64; 64] {
        ...
        for square_index in 0..64 {
            let move_squares: u64 = generate_move_bitboard(square_index, (increments).to_vec(), 0, false, false);
...
static PIECE_INCREMENTS_TABLE: Lazy<[Vec<isize>; 6]> = Lazy::new(|| {
    ...
    table[PieceType::Queen as usize] =
        table[PieceType::Bishop as usize].iter().chain(&table[PieceType::Rook as usize]).cloned().collect();
...
impl<P: MoveProcessor + std::any::Any> MoveGeneratorImpl<P> {
        ...
        P: MoveProcessor,
    {
        ...
        for piece_type in
            [PieceType::King, PieceType::Knight, PieceType::Bishop, PieceType::Rook, PieceType::Queen, PieceType::Pawn]
        {
...
fn square_attacks_finder_internal(
    ...
    for piece_type in [PieceType::Bishop, PieceType::Rook] {
        ...
        for square_index in square_iterator {
            if (enemy_queens | position.board().bitboard_by_color_and_piece_type(attacking_color, piece_type))
                & (1 << square_index)
                != 0
            {
...
mod tests {
    ...
    fn test_white_pawns_can_be_promoted() {
        ...
        assert_eq!(
            *moves.get(0).unwrap(),
            Move::Promotion { base_move: BaseMove::new(50, 58, false), promote_to: PieceType::Queen }
        );
        assert_eq!(
            *moves.get(1).unwrap(),
            Move::Promotion { base_move: BaseMove::new(50, 58, false), promote_to: PieceType::Knight }
        );
        assert_eq!(
            *moves.get(2).unwrap(),
            Move::Promotion { base_move: BaseMove::new(50, 58, false), promote_to: PieceType::Rook }
        );
        assert_eq!(
            *moves.get(3).unwrap(),
            Move::Promotion { base_move: BaseMove::new(50, 58, false), promote_to: PieceType::Bishop }
        );
    }
    ...
    fn test_black_pawns_can_be_promoted() {
        ...
        assert_eq!(
            *moves.get(0).unwrap(),
            Move::Promotion { base_move: BaseMove::new(14, 6, false), promote_to: PieceType::Queen }
        );
        assert_eq!(
            *moves.get(1).unwrap(),
            Move::Promotion { base_move: BaseMove::new(14, 6, false), promote_to: PieceType::Knight }
        );
        assert_eq!(
            *moves.get(2).unwrap(),
        ...
        assert_eq!(
            *moves.get(3).unwrap(),
            Move::Promotion { base_move: BaseMove::new(14, 6, false), promote_to: PieceType::Bishop }
        );
    }
    ...
    fn test_pawns_can_be_promoted_by_capturing() {
        ...
        assert_eq!(
            *moves.get(1).unwrap(),
            Move::Promotion { base_move: BaseMove::new(14, 5, true), promote_to: PieceType::Knight }
        );
        assert_eq!(
            *moves.get(2).unwrap(),
        ...
        assert_eq!(
            *moves.get(3).unwrap(),
            Move::Promotion { base_move: BaseMove::new(14, 5, true), promote_to: PieceType::Bishop }
        );
        ...
        assert_eq!(
            *moves.get(5).unwrap(),
            Move::Promotion { base_move: BaseMove::new(14, 7, true), promote_to: PieceType::Knight }
        );
        assert_eq!(
            *moves.get(6).unwrap(),
        ...
        assert_eq!(
            *moves.get(7).unwrap(),
            Move::Promotion { base_move: BaseMove::new(14, 7, true), promote_to: PieceType::Bishop }
        );
    ...
    fn test_white_king_moves() {
        ...
        assert_eq!(
            **castling_moves.get(0).unwrap(),
            Move::Castling { base_move: BaseMove::new(sq!("e1"), sq!("g1"), false), board_side: BoardSide::KingSide }
        );
        assert_eq!(
            **castling_moves.get(1).unwrap(),
            Move::Castling { base_move: BaseMove::new(sq!("e1"), sq!("c1"), false), board_side: BoardSide::QueenSide }
        );
    }
    ...
    fn test_black_king_moves() {
        ...
        assert_eq!(
            **castling_moves.get(0).unwrap(),
            Move::Castling { base_move: BaseMove::new(sq!("e8"), sq!("g8"), false), board_side: BoardSide::KingSide }
        );
        assert_eq!(
            **castling_moves.get(1).unwrap(),
            Move::Castling { base_move: BaseMove::new(sq!("e8"), sq!("c8"), false), board_side: BoardSide::QueenSide }
        );
    }
