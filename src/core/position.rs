...
struct PositionHashes {
    pub board_hashes_table:
        [[[u64; PositionHashes::NUM_SQUARES]; PositionHashes::NUM_PIECE_TYPES]; PositionHashes::NUM_COLORS],
...
static POSITION_HASHES: Lazy<PositionHashes> = Lazy::new(|| {
    ...
    let mut board_hashes_table: [[[u64; PositionHashes::NUM_SQUARES]; PositionHashes::NUM_PIECE_TYPES];
        PositionHashes::NUM_COLORS] =
        [[[0; PositionHashes::NUM_SQUARES]; PositionHashes::NUM_PIECE_TYPES]; PositionHashes::NUM_COLORS];
...
impl Position {
    ...
    fn create_initial_hash(&self) -> u64 {
        ...
        initial_hash ^=
            POSITION_HASHES.castling_hashes_table[Position::castling_rights_as_u8(&self.castling_rights) as usize];

        if move_gen::is_en_passant_capture_possible(self) {
            initial_hash ^=
                POSITION_HASHES.en_passant_capture_square_hashes_table[self.en_passant_capture_square.unwrap()];
        }
        initial_hash
    }
    ...
    fn put_piece(&mut self, square_index: usize, piece: Piece) {
        ...
        self.hash_code ^=
            POSITION_HASHES.board_hashes_table[piece.piece_color as usize][piece.piece_type as usize][square_index];
    }
    ...
    pub fn make_move(&mut self, mov: &Move) -> Option<UndoMoveInfo> {
        fn make_en_passant_move(position: &mut Position, undo_move_info: &mut UndoMoveInfo, base_move: &BaseMove) {
            ...
            position.remove_piece(
                (undo_move_info.old_en_passant_capture_square.unwrap() as i32 + forward_pawn_increment) as usize,
            );
        }
        ...
        fn make_promotion_move(
            ...
            undo_move_info.captured_piece_type =
                position.remove_piece(undo_move_info.mov.get_base_move().to as usize).map(|piece| piece.piece_type);
        ...
        fn make_basic_move(
            ...
            if piece_type == PieceType::Pawn && util::distance(from as isize, to as isize) == 2 {
                ...
                if from
                    == board::CASTLING_METADATA[position.side_to_move as usize][BoardSide::KingSide as usize]
                        .rook_from_square
                {
                    position.castling_rights[position.side_to_move as usize][BoardSide::KingSide as usize] = false;
                } else if from
                    == board::CASTLING_METADATA[position.side_to_move as usize][BoardSide::QueenSide as usize]
                        .rook_from_square
                {
...
mod tests {
    ...
    fn test_general_usability() {
        let position: Position = Position::new(Board::new(), PieceColor::Black, "KQkq".to_string(), Some(31), 99, 50);
