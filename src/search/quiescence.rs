...
use crate::search::negamax::{increment_node_counter, SearchContext, DRAW_SCORE, MAXIMUM_SCORE, MAXIMUM_SEARCH_DEPTH};
...
// with delta pruning
fn static_exchange_evaluation(position: &Position, mov: &Move) -> i32 {
    let attacked_square = mov.get_base_move().to as usize;
    let attacking_square = mov.get_base_move().from as usize;
    ...
    if let Some(discovered_attacker_square) =
        find_discovered_attacker(position, attacked_square as isize, attacking_square as isize, side_to_move, occupied)
    {
...
fn find_discovered_attacker(
    ...
    if let Some(square_increment) = find_square_increment(target_square, previous_attacker_square) {
        let piece_type =
            if square_increment.abs() == 8 || square_increment == 0 { PieceType::Rook } else { PieceType::Bishop };
        ...
        while util::on_board(previous_attacker_square, square_index) {
            if (1 << square_index) & occupied != 0 {
                ...
                let bitboard =
                    bitboards_for_color[piece_type as usize] | bitboards_for_color[PieceType::Queen as usize];
...
mod tests {
    ...
    fn test_find_discovered_attacker() {
        ...
        let position: Position = Position::from(fen);
        let square_index =
            find_discovered_attacker(&position, sq!("d8"), sq!("d2"), White, position.board().bitboard_all_pieces());
        assert_eq!(square_index, Some(sq!("d1")));

        let fen = "4k3/5r2/8/3B3b/8/1Q6/8/4K3 w - - 0 1";
        let position: Position = Position::from(fen);
        let square_index =
            find_discovered_attacker(&position, sq!("f7"), sq!("d5"), White, position.board().bitboard_all_pieces());
    ...
    mod q_search {
        ...
        fn create_search_context(transposition_table: &mut TranspositionTable) -> SearchContext {
            SearchContext::new(
                transposition_table,
        ...
        #[test]
        fn test_only_kings() {
            let mut transposition_table = TranspositionTable::new(4);
            let mut search_context = create_search_context(&mut transposition_table);
            let fen = "4k3/8/8/8/8/8/8/4K3 w - - 0 1";
            let mut position: Position = Position::from(fen);
            let score = quiescence_search(&mut position, 0, &mut search_context, -MAXIMUM_SCORE, MAXIMUM_SCORE);
            assert_eq!(score, 0);
        }
        ...
        fn test_queening_by_capturing() {
            ...
            let score = quiescence_search(
                &mut position,
                0,
                &mut create_search_context(&mut transposition_table),
                -MAXIMUM_SCORE,
                MAXIMUM_SCORE,
            );
            assert_eq!(score, 903);
