...
use crate::search::negamax::{Search, DRAW_SCORE, MAXIMUM_SCORE, MAXIMUM_SEARCH_DEPTH};
...
impl<'a> Search<'a> {
    pub fn quiescence_search(&mut self, ply: u8, alpha: i32, beta: i32) -> i32 {
        ...
        for mov in captures {
            ...
            if let Some(undo_move_info) = self.position.make_move(&mov) {
                let score = -self.quiescence_search(ply, -beta, -alpha);
                self.position.unmake_move(&undo_move_info);
                if score >= beta {
                    return score;
                }
...
mod tests {
    ...
    fn test_generate_sorted_captures() {
        ...
        let moves = Search::generate_sorted_quiescence_moves(&position);
    ...
    mod q_search {
        ...
        fn test_only_kings() {
            ...
            let mut search = create_search(&mut position, &mut transposition_table);
        ...
        fn test_generated_sorted_quiescence_moves() {
            ...
            let position: Position = Position::from(fen);
            let quiescence_moves = Search::generate_sorted_quiescence_moves(&position);
